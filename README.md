...
Before submitting, each member should:
- Read through the whole file and understand every part, not just the part
  they personally typed — you may be asked to explain any of it.
- Try changing something small (e.g. the discount threshold or rate,
  the max cart size, or the shop name) to confirm you understand how it
  works, not just that it runs.
- Test edge cases together: an empty cart at checkout, entering a negative
  price, entering text instead of a number at a prompt.
